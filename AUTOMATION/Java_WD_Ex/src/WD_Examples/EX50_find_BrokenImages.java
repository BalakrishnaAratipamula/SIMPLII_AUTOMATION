...
public class EX50_find_BrokenImages { /////-----https://youtu.be/6NXZQYs-Eig (other method 'jsup' also there )////image is not loaded properly is called broken images
	...
	public static void verifyBrokenImage(String imgUrl) { ///////////////same as links
		try {
			...
			if(httpurlconnection.getResponseCode()==200) { 
				System.out.println(imgUrl+"  -  "+httpurlconnection.getResponseCode()+"  -  "+httpurlconnection.getResponseMessage()); //by default will get response message as 'OK'
			}
			else{ //if response code is not equal to '200' /////'err' to print error msg in RED colour
				System.err.println(imgUrl+"  -  "+httpurlconnection.getResponseCode()+"  -  "+httpurlconnection.getResponseMessage()+" - "+HttpURLConnection.HTTP_NOT_FOUND); //by default if link not found will get response message as 'Not Found'
			}	
		}
		catch(Exception e) {
